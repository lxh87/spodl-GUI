...
class SpotifyMetadataHandler:
    ...
    def get_metadata(self, url_or_query: str) -> Optional[Dict[str, Any]]:
        ...
        Fields returned:
            - name: Display name (track/album/playlist name)
            - type: 'track', 'album', 'playlist', 'artist', or 'query'
            ...
            - url: Spotify URL
            - track_count: Number of tracks (for albums/playlists)
            - duration: Duration in seconds (for tracks)
            - explicit: Explicit flag (for tracks)
            - popularity: Popularity score 0-100 (for tracks)
            - cover_url: Album/playlist cover image URL
            - description: Description (for playlists)
            - author_name: Playlist author name
            - isrc: ISRC code (for tracks)
            - songs: List of Song objects (for albums/playlists)
        """
        if not url_or_query:
            return None
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("METADATA HANDLER TEST")
    print("="*60)

    print(f"\nSpotDL modules available: {SPOTDL_AVAILABLE}")

    if not SPOTDL_AVAILABLE:
        print("\nNote: SpotDL modules not available.")
        print("Will use subprocess mode (requires 'spotdl' command available)")

    # Test template formatting (works without API)
    print("\n" + "-"*60)
    print("Template Formatting Test (No API Required)")
    print("-"*60)
    ...
    print("\nSample metadata:")
    print(f"  Name: {sample_metadata['name']}")
    print(f"  Artist: {sample_metadata['artist']}")
    print(f"  Album: {sample_metadata['album']}")
    print(f"  Year: {sample_metadata['year']}")
    ...
    # Test sanitization
    print("\n" + "-"*60)
    print("Folder Name Sanitization Test")
    print("-"*60)

    test_names = [
        "Artist - Album (2024)",
        "Artist/Album: Special",
        "Name with <brackets> and |pipes|",
        "   Extra   Spaces   ",
    ]

    print("\nSanitization results:")
    for name in test_names:
        sanitized = handler.sanitize_folder_name(name)
        print(f"\n  Input:  {name}")
        print(f"  Output: {sanitized}")

    print("\n" + "-"*60)
    print("\nNote: To test actual Spotify API metadata fetching:")
    print("  1. Ensure SpotifyClient is initialized (handled by spotdl)")
    print("  2. Run: python test_enhanced_metadata.py")
    print("  3. Or use the GUI which handles authentication automatically")
